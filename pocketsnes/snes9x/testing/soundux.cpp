...
#define VOL_DIV8_SHIFT 15
#define VOL_DIV16 0x0080
#define VOL_DIV16_SHIFT 7
...
//	freq0 = (unsigned long) ((double) freq0 * 0.985);
	...
	for (uint32 I = 0; I < (uint32) sample_count; I++)
	{
		...
		if (ch->type == SOUND_SAMPLE)
		{
		    ...
		    {
			ch->interpolate = ((ch->next_sample - ch->sample) * 
					   (long) freq) >> FIXED_POINT_SHIFT;
			ch->sample = (int16) (ch->sample + (((ch->next_sample - ch->sample) * 
					   (long) (ch->count)) >> FIXED_POINT_SHIFT));
		    }		  
		    else
			ch->interpolate = 0;
		}
...
void S9xMixSamplesO (uint8 *buffer, int sample_count, int byte_offset)
{
	...
	else
	{
		...
		if (so.stereo)
		{
			...
			for (J = 0; J < sample_count; J++)
			{
			    ...
			    I = (MixBuffer [J] * 
				 ...
				 E * SoundData.echo_volume [J & 1]) >> VOL_DIV16_SHIFT;
		...
		else
		{
			...
			for (J = 0; J < sample_count; J++)
			{
			    ...
			    I = (MixBuffer [J] *
				 ...
				 E * SoundData.echo_volume [0]) >> VOL_DIV16_SHIFT;
			    CLIP16(I);
			    ((signed short *) buffer)[J + O] = I;
			}
		    }
		...
		{
		    I = (MixBuffer [J] * 
			 SoundData.master_volume [J & 1]) >> VOL_DIV16_SHIFT;
	...
	if (so.encoded)
	{
	    for (J = 0; J < sample_count; J++)
	    {
		I = (MixBuffer [J] * SoundData.master_volume_left) >> VOL_DIV16_SHIFT;
		CLIP16(I);
...
#endif
	{
		...
		if (so.stereo)
		{
			...
			for (J = 0; J < sample_count; J++)
			{
			    ...
			    I = (MixBuffer [J] * 
				 SoundData.master_volume [J & 1] +
				 E * SoundData.echo_volume [J & 1]) >> VOL_DIV8_SHIFT;
			    CLIP8(I);
			    buffer [J + O] = I + 128;
			}
		    }
		...
		else
		{
			...
			for (J = 0; J < sample_count; J++)
			{
			    ...
			    I = (MixBuffer [J] * SoundData.master_volume [0] +
				 E * SoundData.echo_volume [0]) >> VOL_DIV8_SHIFT;
			    CLIP8(I);
			    buffer [J + O] = I + 128;
			}
		    }
		...
		{
		    I = (MixBuffer [J] * 
			 SoundData.master_volume [J & 1]) >> VOL_DIV8_SHIFT;
		    CLIP8(I);
		    buffer [J + O] = I + 128;
		}
	    }
...
bool8_32 S9xInitSound (int rate, bool8_32 stereo, int buffer_size)
{
    so.sound_fd = -1;
    so.sound_switch = 255;

    so.playback_rate = rate;
    ...
    if (!S9xOpenSoundDevice (rate, stereo, buffer_size))
    {
