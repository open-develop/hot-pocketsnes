		...
		for (Lines = 1; Lines < 8 - VirtAlign; Lines++)
			...
			for (int clip = 0; clip < clipcount; clip++){
				...
				if (Count){
					...
					if (Tile != TileBlank){
						GFX.Z1 = GFX.Z2 = depths [(Tile & 0x2000) >> 13];
						(*DrawClippedTilePtr) (Tile, s, 0, Count & 7, VirtAlign, Lines, &GFX);
					}
				}
		}
    }
			...
			!((GFX.r2130 & 0x30) == 0x10 && IPPU.Clip[1].Count[5] == 0))
		{
		    ...
		    if (pClip->Count [5])
		    {
				...
				for (uint32 y = starty; y <= endy; y++)
				{
					...
					memset ((uint32_t*)(GFX.SubZBuffer + y * GFX_ZPITCH),0,
						IPPU.RenderedScreenWidth);
					// Clear the Zbuffer
					memset ((uint32_t*)(GFX.ZBuffer + y * GFX_ZPITCH),0,
						IPPU.RenderedScreenWidth);

					// if there is clipping then clear subscreen to a black color
					if (IPPU.Clip [0].Count [5])
					{
						memset ((uint32_t*)(GFX.SubScreen + y * GFX_PITCH), black, IPPU.RenderedScreenWidth);
					}
...
//#undef RC_OPTIMIZED
			...
			for (uint32 y = starty; y <= endy; y++)
			{
				...
				memset ((uint32_t*)(GFX.SubZBuffer + y * GFX_ZPITCH), 0x01010101,
				    IPPU.RenderedScreenWidth);
			    if (IPPU.Clip [0].Count [5])
			    {
				// Blast, have to clear the sub-screen to the fixed-colour
				...
				memset ((uint32_t*)(GFX.SubScreen + y * GFX_PITCH), fixedColour,
				    IPPU.RenderedScreenWidth);
			    }
			}
#endif
		...
		else
		{
		    ...
		    if (IPPU.Clip [0].Count[5])
		    {
				...
				for (uint32 y = starty; y <= endy; y++)
				{
					memset ((uint32_t*)(GFX.Screen + y * GFX_PITCH), black,
						IPPU.RenderedScreenWidth);
		    ...
		    else
		    {
				...
				for (uint32 y = starty; y <= endy; y++)
				{
					memset ((uint32_t*)(GFX.Screen + y * GFX_PITCH), back,
						IPPU.RenderedScreenWidth);
				}
#endif
		    }
...
#define DISPLAY(n)	((GFX.r212c & n) || ((GFX.r212d & n) && subadd))
	...
	if (IPPU.LatchedInterlace)
	{
	    ...
	    for (uint32 y = GFX.StartY; y <= GFX.EndY; y++)
	    {
		memcpy ((uint32_t*)(GFX.Screen + (y * 2 + 1) * GFX_PITCH),
