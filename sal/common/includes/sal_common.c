...
static u32 volatile mFb=0;
static u32 mBpp=16;
static u32 mRefreshRate=60;
...
{
	u32 aim=mSb;
	aim--; if (aim>=SOUND_BUFFER_COUNT) aim=SOUND_BUFFER_COUNT-1;
	return aim;
}
...
{
	u32 aim=index;
	aim++; if (aim>=SOUND_BUFFER_COUNT) aim=0;
	return aim;
}
...
u32 sal_AudioRatePrevious(u32 currRate)
{
	s32 x;
	u32 newRate;
	for (x=AUDIO_RATE_COUNT-1; x>=0; x--)
	{
		newRate=mAudioRateLookup[x];
		if(newRate<currRate) break;
	}
